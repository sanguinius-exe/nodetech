...
## Web terminal ([web/index.html](web/index.html))

A single self-contained HTML page that runs the actual game — not a reimplementation — inside the browser via [Pyodide](https://pyodide.org/) (CPython compiled to WebAssembly). On load it fetches `main.py`/`node.py`/`country.py`/`division.py`/`database.py` as plain text, writes them into Pyodide's virtual filesystem, and does a plain `import main as game` — the exact same `World` class and `run_command()` dispatcher the CLI uses.
...
- **Web terminal has no persistent state**: reloading the page restarts the Pyodide interpreter from scratch and any unsaved world is gone — there's no autosave, so "Download current save" before closing the tab is the only way to keep one.
